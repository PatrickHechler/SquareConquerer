...
public class WorkerUnit extends SimpleUnit {
	
	private static BufferedImage img;
	
	WorkerUnit(UUID uuid, Map<String, Value> vals) {
		super(uuid, vals);
	}
	...
	private static synchronized void loadImg() {
		if (img != null) return;
		try {
			img = ImageIO.read(WorkerUnit.class.getResource("/img/unit/worker.png"));
		} catch (IOException e) {
			throw new IOError(e);
		}
	}
	...
	protected double attackStrength(Entity<?, ?> enemy, int oldLives) throws TurnExecutionException {
		return 0.3D * super.attackStrength(enemy, oldLives);
	}
