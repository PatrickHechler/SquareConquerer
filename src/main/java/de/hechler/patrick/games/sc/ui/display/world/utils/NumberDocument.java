...
public class NumberDocument extends PlainDocument {
		...
		try {
			...
			if (val < this.minNum || val > this.maxNum) {
				if (this.minNum > 9 && val >= 0) {
					super.replace(0, getLength(), Integer.toString(this.minNum), a);
				}
				return;
			}
	 ...
	 * <p>
	 * note that if the document contains an empty string this method returns 0 (even if 0 is outside of the allowed bounds)<br>
	 ...
	 * returns the current number of this document
	 * <br>
	 * if the document contains an empty string this method returns <code>empty</code> (even if <code>empty</code> is outside of the allowed bounds)
	 * 
