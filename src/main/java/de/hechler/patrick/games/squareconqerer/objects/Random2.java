...
/**
 * this is a clone of {@link java.util.Random}.
 * differences are that there is a {@code getCurrentSeed()} and some unneeded
 * stuff is removed. Also this class is {@code final}
 * <p>
...
public final class Random2 implements RandomGenerator, java.io.Serializable {
	 ...
	 *           result
	 *           in an uneven distribution (due to the fact that 2^31 is not
	 *           divisible
	 *           by n). The probability of a value being rejected depends on n. The
	 *           worst case is n=2^30+1, for which the probability of a reject is
	 *           1/2,
	 *           and the expected number of iterations before the loop terminates is
	 *           2.
	 *           <p>
	...
	@Override
	public int nextInt(int bound) {
		if (bound <= 0) throw new IllegalArgumentException("bad bound: " + bound);
		int r = next(31);
		int m = bound - 1;
	 ...
	 *           randomly
	 *           chosen bits, then the algorithm shown would choose {@code double}
	 *           values
	 ...
	 * 
	 *           This might seem to be equivalent, if not better, but in fact it
	 *           introduced a large nonuniformity because of the bias in the
	 *           rounding of
	 *           floating-point numbers: it was three times as likely that the
	 *           low-order
	 *           bit of the significand would be 0 than that it would be 1! This
	 *           nonuniformity probably doesn't matter much in practice, but we
	 *           strive
	 *           for perfection.]
	 *
	 ...
	 * }</pre>
	 *
	 *           This uses the <i>polar method</i> of G. E. P. Box, M. E. Muller,
	 *           and
	 *           G. Marsaglia, as described by Donald E. Knuth in <cite>The Art of
	 *           Computer Programming, Volume 2, third edition: Seminumerical
	 *           Algorithms</cite>,
